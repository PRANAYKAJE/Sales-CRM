...
export default function LeadDetail() {
  ...
  const fetchLeadData = async () => {
    try {
      ...
      setDeals(allDeals.filter((deal) => deal.leadId === id || deal.lead?._id === id))
      
      const allActivities = activitiesRes.data || []
      setActivities(allActivities.filter((activity) => activity.leadId === id || activity.lead?._id === id))
    } catch (err) {
  ...
  const getStageBadge = (stage) => {
    const classes = {
      'Prospecting': 'bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300',
      'Qualification': 'bg-purple-100 dark:bg-purple-900/50 text-purple-700 dark:text-purple-300',
      'Proposal': 'bg-orange-100 dark:bg-orange-900/50 text-orange-700 dark:text-orange-300',
      'Negotiation': 'bg-yellow-100 dark:bg-yellow-900/50 text-yellow-700 dark:text-yellow-300',
      'Closed': 'bg-emerald-100 dark:bg-emerald-900/50 text-emerald-700 dark:text-emerald-300',
    }
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-card p-6 border border-gray-100 dark:border-gray-700">
            <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-4">Activities</h3>
            {activities.length === 0 ? (
                ...
                {activities.map((activity) => (
                  <div key={activity._id} className="flex gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">{formatDate(activity.date)}</p>
                    </div>
                  </div>
                ))}
              </div>
